> **Historical record — 2026-04-19.** This document is the original 7-specialist panel review that seeded issues #111–#131. It is preserved unchanged as a provenance record. Many of the P0 and P1 items it raised were resolved in subsequent releases: the integration gained `allow_private_urls` (unblocking the default LAN install path); the public-CDN mirror path was demoted to a "last resort" with explicit privacy warnings; `schema_version` and `properties.id` were added to the zones format; `GET /zones.json` was gated behind `save_token`; accessibility fixes landed in 0.2.27; the self-hosted Leaflet / Leaflet-Draw path was completed in 0.2.32; amd64-only arch smoke testing (sre-reliability, P2 item 18) was addressed by native aarch64 build+boot+Playwright in `build.yml`, and the Supervisor-level boot gate (P0 item 5) is partly addressed: ADR 0001's Supervisor pilot (`.github/workflows/supervisor.yml`) runs the add-on under a real stable Supervisor nightly and on demand, but it is not yet a required check and `release.yml` does not call it, so a release can still publish without it (ADR 0001 row 5) — see TESTING.md for the current layout. See the CHANGELOG for what shipped in each release.
